...
# filter out response rate at subcategory level <24.5% for school average
      ...
      filepath = Rails.root.join("tmp", "reports", "survey_item_report_by_grade.csv")
      write_csv(data:, filepath:)
      data
    end

    def self.create_item_report(school:, academic_year:)
      ...
      FileUtils.mkdir_p Rails.root.join("tmp", "reports")
      filepath = Rails.root.join("tmp", "reports", "survey_item_report_by_item.csv")
      write_csv(data:, filepath:)
      data
    end
